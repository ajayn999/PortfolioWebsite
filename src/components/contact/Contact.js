...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_beovmgz', 'template_ecx7wvp', form.current, 'D1_fODzIP4n68F955')
      .then((result) => {
            ...
            <form className="w-full flex flex-col gap-4 lgl:gap-6 py-2 lgl:py-5" ref={form} onSubmit={sendEmail}>
              ...
              <div className="w-full flex flex-col lgl:flex-row gap-10">
                <div className="w-full lgl:w-1/2 flex flex-col gap-4">
                  ...
                  <input type="text" name="user_name" onChange={(e) => setUsername(e.target.value)}
                    value={username1}
                    className={`${errMsg1 === "Username is required!" &&
                      "outline-designColor"
                      } contactInput`}
                  />
                </div>
                <div className="w-full lgl:w-1/2 flex flex-col gap-4">
                  ...
                  <input type="text" name="user_number" onChange={(e) => setPhoneNumber(e.target.value)}
                    value={phoneNumber1}
                    className={`${errMsg1 === "Phone number is required!" &&
                      "outline-designColor"
                      } contactInput`}
                  />
                </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <input type="email" name="user_email" onChange={(e) => setEmail(e.target.value)}
                  value={email1}
                  className={`${errMsg1 === "Please give your Email!" &&
                    "outline-designColor"
                    } contactInput`}
                />
              </div>
              <div className="flex flex-col gap-4">
                ...
                <input type="text" name="user_subject" onChange={(e) => setSubject(e.target.value)}
                  value={subject1}
                  className={`${errMsg1 === "Plese give your Subject!" &&
                    "outline-designColor"
                    } contactInput`}
                />
              </div>
              <div className="flex flex-col gap-4">
                ...
                <textarea name="message" onChange={(e) => setMessage(e.target.value)}
                  value={message1}
                  className={`${errMsg1 === "Message is required!" && "outline-designColor"
                    } contactTextArea`}
                  cols="30"
                  rows="8"
                />
              ...
              <div>
                <button type="submit" value="Send" onClick={handleSend}
                 className="w-full h-12 bg-[#141518] rounded-lg text-base text-gray-400 tracking-wider uppercase hover:text-white duration-300 hover:border-[1px] hover:border-purple-800 border-transparent"
                >
                  Send_Message!
                </button>
              </div>
